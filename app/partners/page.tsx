...
export default function PartnersPage() {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        ...
        const formattedClients = (strapiData?.data || []).map((item: any) => {
  ...
  const filteredClients = useMemo(() => {
    if (activeSector === "All") return clients;
    return clients.filter((c) => c.sector === activeSector);
  }, [clients, activeSector]);
    ...
    <main className="min-h-screen bg-white flex flex-col pt-12">
      ...
      <section className="max-w-7xl mx-auto px-6 py-16 w-full flex-grow">
        <div className="flex flex-col gap-12">
          {/* Sector Filters (Always show full structure) */}
